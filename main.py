...
def finish_chain(cid):
    ...
    bot.send_message(cid,f"✅ Обязательный блок собран.{warn}\n\nТеперь желательные кадры из списка выше — пришлите их одним сообщением или напишите «Готово». Если снять не получается — «не могу».")
...
def text(m):
    ...
    if s["stage"]=="name":
        ...
        s["closed"]=[]
        s["stage"]="chain"
        bot.send_message(cid,f"Принято, проверяем: {t}.\n\n{s['shots']}\n\nНачинаем с обязательных кадров — буду подсказывать каждый шаг. Лучше прикреплять как документ (📎 → «Документ»), обычные фото тоже принимаю.\n\n➡️ Шаг 1/{len(s['queue'])}: {s['queue'][0]}")
        return
    if s["stage"]=="chain":
