...
САМОПРОВЕРКА перед выдачей отчёта (обязательно):
(а) число ❌ в деталях равно числу ❌ в основании плашки;
(б) НЕ упоминай дополнительные проверки и перепроверки — на момент отчёта их ещё не было;
(в) каждый процитированный пункт TIER A/B действительно соответствует типу маркера, а не растянут под него.
...
2) Пять деталей СТРОГО в порядке 01→05. Строка детали СТРОГО в формате: «NN <знак> Название: обоснование». Примеры: «01 ✅ Упаковка и полиграфия: шрифт ровный и чёткий…»; «03 ❌ Маркировка и батч-код: код на флаконе 45L310, на коробке 45L39 — не совпадают (п.2 TIER A)». Знаки: ✅ проверено / ⚠️ сомнение / ❌ маркер / ➖ не проверяется (именно ➖, не тире). В ❌-обосновании — конкретика и номер пункта закрытого списка; без конкретики ставь ✅ или ⚠️.
...
"3. После подтверждения пригодности фото — оплата: 500 ₽ (Стандартный, до 3 ч) или 1000 ₽ (Экспресс, до 15 минут, PDF-отчёт с фрагментами).\n"
...
def build_pdf(s,rep,crops):
    ...
    try:
        ...
        tail=""
        if "05" in det:
            m2=re.search(r"(?m)^(Аромат и состав|Результат —|Отчёт не подтверждает|Каждый пункт|Разбор выполнен только по флакону)",det["05"])
            if m2:
...
def demote_detail(rep,n):
    lines=rep.split("\n")
    for i,l in enumerate(lines):
        if re.match(r"^\s*"+n+r"\b",l):
            lines[i]=l.replace("❌","⚠️",1).replace("маркер","сомнение",1)+" Дополнительная проверка не подтвердила маркер; статус снижен до сомнения."
            break
    return "\n".join(lines)

def postprocess_verdict(report):
    d=parse_details(report)
    red_count=sum(1 for v in d.values() if v=="❌")
    if red_count>=2:
    ...
    wrong=["🔴 «Выявлены признаки несоответствия оригиналу»","⚠️ «Есть сомнения в соответствии оригиналу»","🟢 «Признаков несоответствия оригиналу не выявлено»",
    "🔴 Выявлены признаки несоответствия оригиналу","⚠️ Есть сомнения в соответствии оригиналу","🟢 Признаков несоответствия оригиналу не выявлено"]
    for w in wrong:
        if w!=phrase:
    ...
    if red_count<2:
        report=re.sub(r'Основание:[^\n]*два независимых ❌[^\n]*','Основание: один ❌-маркер по деталям',report)
    return report
...
def verify_reds(cid,s,rep,details,model):
    ...
    for n in [k for k,v in details.items() if v=="❌"]:
        ...
        try:
            ...
            continue
        up=vr.upper()
        verdict="ОПРОВЕРГАЮ" if "ОПРОВЕРГАЮ" in up else ("ПОДТВЕРЖДАЮ" if "ПОДТВЕРЖДАЮ" in up else "НЕЯСНО")
        logging.info("VERIFY cid=%s n=%s item=%s verdict=%s",cid,n,item,verdict)
        if verdict!="ПОДТВЕРЖДАЮ":
            rep=demote_detail(rep,n)
            details[n]="⚠️"
    return rep,details
...
def add_image(m):
    ...
    # Перепроверка только документом
    if s["stage"]=="done" and comp:
        send_html(cid,"📥 Для перепроверки принимаю фото ТОЛЬКО документом: Telegram сжимает обычные фото, и перепроверка будет неточной. Отправьте документом:\n\n"+DOC_HELP)
        return
    ...
    # Медиагруппа на перепроверке
    if s["stage"]=="recheck":
        s["rc_photo"]=b64
        bot.send_message(cid,"📥 Фото обновлено. Какой пункт перепроверить? Напишите номер от 01 до 05.")
        return
...
def text(m):
    ...
    if s["stage"]=="recheck":
        ...
        newst=next((e for e in ("❌","⚠️","✅","➖") if e in line),None)
        if newst is None:
            s["stage"]="done"
            bot.send_message(cid,"Не смог распознать статус перепроверки — пункт оставлен без изменений. Попробуйте прислать более чёткое фото.")
            return
        old=s["details"].get(n,"➖")
        s["details"][n]=newst
    ...
    if s["stage"]=="name":
        ...
        if corrected.lower()!=t.lower():
            s["name"]=corrected
            bot.send_message(cid,f"Принято: {corrected} (исправлено из «{t}»). Если неверно — напишите «Начать заново».",reply_markup=types.ReplyKeyboardRemove())
        else:
            bot.send_message(cid,f"Принято: {t}.",reply_markup=types.ReplyKeyboardRemove())
...
def cb(c):
    ...
    if c.data in ("std","exp"):
        ...
        model=s.get("model") or QWEN_MODEL
        try:
            rep=ask_qwen(s["photos"],MODE2.format(name=s["name"] or "?")+note+tariff_note,model,timeout=150,attempts=2)
        except Exception:
