...
- Никаких «100% гарантия», «официальный чек-лист». Вердикт — только из триады с формулировками ниже.
...
1) батч на флаконе нечитаемый, стёртый или отсутствует;
...
СПИСОК «НЕ ПРОВЕРЯЕТСЯ»: детали из этого списка помечай «➖ не проверяется», выводов по ним не делай; вердикт выноси только по проверенным и укажи это в плашке. Аромат и состав содержимого по фото не верифицируются никогда."""

MODE_BOX="""Продукт: «{name}». Если название похоже на опечатку — попробуй понять, что имелось в виду.
Ответь СТРОГО тремя строками:
...
Правила списка:
- Названия шагов — короткие, без пояснений и БЕЗ слова «макро». Используй названия кадров, не названия деталей. Примеры: «Батч-код на флаконе», «Флакон спереди с этикеткой», «Батч-код на коробке (при наличии)», «Сторона коробки с названием (при наличии)», «Дно флакона», «Распылитель и завальцовка», «Трубочка на просвет», «Крышка». Без повторов.
...
- Порядок по важности: батч-код на флаконе → флакон спереди с этикеткой → батч-код на коробке (при наличии) → сторона коробки с названием (при наличии) → дно флакона → распылитель и завальцовка → трубочка на просвет → крышка.
...
ВАЖНО: фото дна флакона — это шаг «Батч-код на флаконе» (код гравируется на дне), пока «Батч-код на флаконе» есть в оставшихся шагах. Относи фото дна к шагу «Дно флакона» ТОЛЬКО если «Батч-код на флаконе» уже снят или отсутствует в списке.
...
TIER A (ур.2, всегда активен):
(1) батч на флаконе нечитаем/стёрт/отсутствует;
...
START_TEXT=("👋 Legit Check Perfume — проверка признаков подлинности парфюмерии по фото.\n\n"
"Как это работает:\n"
"1. Напишите название аромата и бренд — составим список необходимых кадров под ваш флакон.\n"
...
"Напишите название аромата и бренд.\nПример: «Dior Sauvage», «Chanel №5»")
...
"Чтобы начать, напишите название аромата и бренд.\nПример: «Dior Sauvage», «Chanel №5».")
...
BASE_LISTS={
"spray":["Батч-код на флаконе","Флакон спереди с этикеткой","Дно флакона","Распылитель и завальцовка","Трубочка на просвет","Крышка"],
"roll":["Батч-код на флаконе","Флакон спереди с этикеткой","Дно флакона","Ролик и горлышко","Крышка"],
"splash":["Батч-код на флаконе","Флакон спереди с этикеткой","Дно флакона","Горлышко и ограничитель","Крышка"],
"sample":["Батч-код на флаконе","Флакон спереди с этикеткой","Дно флакона","Крышка или распылитель"],
}
...
def hint_for(step,ff):
    ...
    if "батч" in s:
        return "Код выгравирован или напечатан на дне флакона, иногда — на боку у основания. Переверните флакон и снимите дно крупным планом, чтобы текст читался."+BATCH_NO_NOTE
...
def add_image(m):
    ...
    if s["stage"]=="name":
        bot.send_message(cid,"Сначала напишите название аромата и бренд.")
        return
    if s["stage"]=="tariffs":
...
def audit(cid):
    ...
    if not (crit("01") and crit("02") and crit("03")):
        ...
        bot.send_message(cid,"По имеющимся фото вердикт вынести невозможно: не хватает критических деталей (упаковка, флакон или маркировка). Услуга не оказывается, оплата не запрашивается. Добавьте читаемые фото или начните заново.",reply_markup=kb)
        return
