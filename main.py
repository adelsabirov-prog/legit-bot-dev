...
Без других вступлений. Без звёздочек и маркдауна — обычный текст."""
...
MISSING: [через запятую детали из 6, не покрытые читаемыми фото, кроме тех, что клиент не может предоставить; «нет», если все покрыты]"""
...
1) Плашка: эмодзи (🔴/⚠️/🟢) + вердикт словами + 1-2 предложения основания + отдельной строкой: «Вердикт вынесен по деталям: [список]».
2) Шесть деталей, по каждой строка статуса строго из набора (✅ проверено / ⚠️ сомнение / ❌ маркер / ➖ не проверяется) и 1-2 предложения обоснования:
