...
В плашке ОБЯЗАТЕЛЬНО отдельной строкой перечисли: «Проверены: [номера деталей со статусами ✅/⚠️/❌]; не проверялись: [номера со статусом ➖]». Перечисли ВСЕ пять деталей, ни одну не пропусти. Знак непроверяемых — именно ➖, не тире.
...
def batch_no_note(obj):
    return " Если кода на флаконе нет совсем — напишите «нет», проверим "+("коробку и остальные детали." if obj=="both" else "остальные детали.")
...
def hint_for(step,ff,obj="both"):
    ...
    if "батч" in s:
        return "Код на дне флакона: гравировка, печать или наклейка; иногда — на боку у основания. Код может быть прозрачным и малозаметным — посмотрите дно на свету. Переверните флакон и снимите дно крупным планом, чтобы читался текст и были видны швы стекла. Снимите при рассеянном свете, без прямого солнца и бликов."+batch_no_note(obj)
    ...
    if "крышк" in s:
        return "Снимите крышку крупным планом, чтобы были видны форма и пластик: сверху и, если удобно, изнутри (1–2 кадра). Надписи на крышке не нужны."
    return ""

def batch_note(s,n):
    obj=s.get("obj") or "both"
    if 1<=n<=len(s["queue"]) and "батч" in s["queue"][n-1].lower() and "короб" not in s["queue"][n-1].lower():
        return "\nЕсли видите, что кода на флаконе нет совсем — напишите «нет», "+("проверим коробку и остальные детали." if obj=="both" else "проверим остальные детали.")
    return ""
