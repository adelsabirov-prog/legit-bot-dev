...
def why_for(step):
    ...
    if "батч" in s and "короб" in s:
        return "Зачем: сверяем код на коробке с кодом на таре — несовпадение выдаёт переупаковку."
    if "батч" in s:
        return "Зачем: код и способ его нанесения — сильные признаки оригинала; по коду сверяем тару и коробку."
    if "короб" in s and ("зад" in s or "дно" in s):
        return "Зачем: на задней и нижней стороне коробки — состав, строка дистрибьютора и штрихкод; проверяем полиграфию."
    if "короб" in s:
        return "Зачем: полиграфия коробки (шрифты, тиснение) часто выдаёт подделку."
    if "текстура" in s:
        return "Зачем: цвет и консистенция средства — дополнительный признак; аномалия бывает маркером."
    if "крышк" in s or "мембран" in s or "пломб" in s:
        return "Зачем: защитная мембрана и посадка крышки показывают, вскрывался ли продукт и есть ли защита."
    if "механизм" in s or "помп" in s or "дозатор" in s or "стик" in s:
        return "Зачем: ход механизма и качество фурнитуры сложно воспроизвести в подделке."
    if "тар" in s:
        return "Зачем: проверяем полиграфию этикетки и качество литья тары — швы, облой, пластик."
    return ""
