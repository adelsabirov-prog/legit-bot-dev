...
"Вердикт выносится по проверенным деталям.\nНепроверенные честно помечаются «не проверяется».")
...
def text(m):
    ...
    if s["stage"]=="name":
        ...
        s["closed"]=[]
        s["stage"]="chain"
        bot.send_message(cid,f"Принято, проверяем: {t}.\n\n{s['shots']}\n\nСобираем кадры по шагам — буду подсказывать каждый и скажу, если нужно переснять. Удобнее сразу? Пришлите все фото из списка одним сообщением — я распределю их по шагам сам. Лучше прикреплять как документ (📎 → «Документ»), обычные фото тоже принимаю. Шаги с «(при наличии)» пропускайте, если коробки или детали нет — просто напишите «нет».\n\n"+step_msg(s,0))
        return
    if s["stage"]=="chain":
        low=t.lower()
