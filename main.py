...
"Как это работает:\n"
"1. Напишите название аромата — составим список необходимых кадров под ваш объект.\n"
...
def hint_for(step,ff):
    ...
    if "батч" in s:
        return "Код на дне флакона: гравировка, печать или наклейка; иногда — на боку у основания. Переверните флакон и снимите дно крупным планом, чтобы читался текст и были видны швы стекла. Снимите при рассеянном свете, без прямого солнца и бликов."+BATCH_NO_NOTE
...
def kb_obj():
    kb=types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("🧴 Флакон",callback_data="obj_bottle"))
    kb.add(types.InlineKeyboardButton("🧴+ Флакон и коробка",callback_data="obj_both"))
    return kb
...
def add_image(m):
    ...
    if s["stage"]=="name":
        ...
        return
    if s["stage"]=="box":
        bot.send_message(cid,"Сначала нажмите кнопку: «Флакон» или «Флакон и коробка».")
        return
    if s["stage"]=="tariffs":
...
def text(m):
    ...
    if s["stage"]=="name":
        ...
        return
    if s["stage"]=="box":
        bot.send_message(cid,"Нажмите кнопку: «Флакон» или «Флакон и коробка».")
        return
    if s["stage"]=="chain":
        low=t.lower()
