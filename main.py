...
- ЗАКРЫТЫЙ СПИСОК ❌-МАРКЕРОВ. ❌ разрешён ТОЛЬКО по этим типам и ни по каким другим:
  1) батч на таре нечитаемый, стёртый или отсутствует;
  2) несовпадение батча на таре и коробке, когда обе видны в кадре;
  3) анахронизм эпохи: батч ↔ строка дистрибьютора — ТОЛЬКО парой и ТОЛЬКО при точном знании бренда;
  4) дефект полиграфии, видимый и процитированный из кадра: плывущий шрифт, кривая печать, опечатка в тексте этикетки;
  5) дефект литья, видимый в кадре: облой, кривые швы, сколы;
  6) конфликт «коробка выглядит оригинальной ↔ тара нет».
  НЕ выдумывай «обязательные элементы» защиты/маркировки бренда вне этого списка. Всё вне списка (фольга/мембрана, «отсутствие повтора батча на фольге», оттенки, складки, следы вскрытия) — НЕ может быть ❌; максимум ⚠️ «слабый признак», и только с конкретикой.
- ❌-маркер — ТОЛЬКО с конкретным основанием: процитируй, что именно читается/видится на фото, и объясни, чему это должно быть у оригинала. Расплывчатые формулировки («видны несоответствия», «такие как…», «вызывает подозрения») БЕЗ конкретики — НЕ маркеры.
- 🔴 — ТОЛЬКО при 2+ независимых ❌ из закрытого списка. Ровно 1 ❌ — строго ⚠️.
- ✅ ставится ТОЛЬКО по детали, которая реально видна в кадре. Деталь не видна или не применима к форм-фактору (например, механизмы у банки) — строго «➖ не проверяется», никогда ✅.
- Батч и дата: форматы вида E27/09/2027, ДД/ММ/ГГГГ, EXP+дата нормальны для разных рынков и сами по себе НЕ маркеры.
- Следы вскрытия и использования (помятая фольга, складки, отпечатки, следы крема, сорванная плёнка) — нормальное состояние вскрытого продукта, НЕ маркеры.
...
ур.2 — общие признаки форм-фактора: качество полиграфии, тиснение, швы и облой литья, посадка крышки и ход механизмов, способ нанесения и шрифт гравировки, согласованность маркировки с упаковкой.
...
- 04-06 — дополнительные: обычно усиливают/ослабляют; аномалия по ним может быть ⚠️, но ❌ — только по закрытому списку.
- Нормальная текстура/содержимое — односторонний маркер: не снимает 🔴/️.
- Конфликт «коробка читается как оригинал, тара — нет» — пункт 6 закрытого списка.
...
Оставшиеся шаги:
{remaining}
На фото косметика или уход, ИЛИ часть их упаковки (этикетка, крышка, мембрана/фольга, текстура средства, батч-код, дно тары)? Если да или похоже — анализируй шаг.
ТИП: не косметика — ТОЛЬКО если на фото явно посторонний предмет: еда, одежда, техника, документ, человек, пустой стол.
...
На фото косметика или уход, ИЛИ часть их упаковки (этикетка, крышка, мембрана/фольга, текстура, батч-код)? Если да или похоже — анализируй.
ТИП: не косметика — ТОЛЬКО если явно посторонний предмет: еда, одежда, техника, документ, человек, пустой стол.
...
КАЛИБРОВКА (строго): ❌ — ТОЛЬКО по закрытому списку: (1) батч на таре нечитаем/стёрт/отсутствует; (2) несовпадение батча таре↔коробка при обеих видимых; (3) анахронизм батч↔дистрибьютор парой при точном знании; (4) процитированный дефект полиграфии из кадра; (5) видимый дефект литья; (6) конфликт коробка↔тара. Всё вне списка (фольга, «нет повтора батча на фольге», оттенки, складки, следы вскрытия) — НЕ ❌, максимум ️ с конкретикой. Не выдумывай «обязательные элементы» бренда вне списка. 🔴 — только при 2+ ❌ из списка; ровно 1 ❌ — ⚠️. Деталь не видна или не применима (механизмы у банки) — «➖ не проверяется», никогда ✅. Форматы даты E27/09/2027 и подобные — НЕ маркеры.
...
2) Шесть деталей, по каждой строка статуса строго из набора (✅ проверено / ⚠️ сомнение / ❌ маркер / ➖ не проверяется; знак для непроверяемых — именно ➖, не тире) и 1-2 предложения обоснования; в ❌-обосновании — конкретика и номер пункта закрытого списка, без неё ставь ✅ или ⚠️.
...
BATCH_NO_NOTE=" Если кода на таре нет совсем — напишите «нет», проверим коробку и остальные детали."
...
def hint_for(step,ff):
    ...
    if "батч" in s:
        base={
        "bank":"Переверните банку и снимите дно крупным планом, чтобы текст читался (код бывает гравировкой по ободку, печатью или наклейкой; иногда — на наклейке сбоку).",
...
def batch_note(s,n):
    if 1<=n<=len(s["queue"]) and "батч" in s["queue"][n-1].lower() and "короб" not in s["queue"][n-1].lower():
        return "\nЕсли видите, что кода на таре нет совсем — напишите «нет», пойдём дальше."
    return ""
...
def st(cid):
    return S.setdefault(cid,{"name":"","photos":[],"shots":"","queue":[],"closed":[],"cannot":[],"last_missing":[],"stage":"name","source":"","tariff":"","ff":"default","pending":-1,"pending_b64":"","retakes":0,"chain_complete":False,"last_closed":-1})
...
def reset(cid):
    S[cid]={"name":"","photos":[],"shots":"","queue":[],"closed":[],"cannot":[],"last_missing":[],"stage":"name","source":"","tariff":"","ff":"default","pending":-1,"pending_b64":"","retakes":0,"chain_complete":False,"last_closed":-1}
...
def start(m):
    parts=m.text.split()
    S[m.chat.id]={"name":"","photos":[],"shots":"","queue":[],"closed":[],"cannot":[],"last_missing":[],"stage":"name","source":parts[1] if len(parts)>1 else "","tariff":"","ff":"default","pending":-1,"pending_b64":"","retakes":0,"chain_complete":False,"last_closed":-1}
