...
- Отсутствие батч-кода на флаконе само по себе — НЕ ❌: у части оригиналов код на дне не гравируется. ❌ по п.1 TIER A — ТОЛЬКО при видимых следах ВМЕШАТЕЛЬСТВА (стёртость, затирание, наклейка поверх гравировки). Код отсутствует без следов вмешательства — максимум ⚠️ с оговоркой, что у части оригиналов кода на дне нет.
- Отсутствие «ожидаемых надписей или логотипов» на коробке — НЕ ❌ без точного знания бренда (это ур.1, дата-гейт). П.7 TIER A — ТОЛЬКО конфликт «коробка выглядит оригинальной ↔ флакон нет». Не выдумывай «ожидаемый дизайн» оригинала.
...
- Каждый ❌ и каждый ️ обязан содержать цитату из кадра: процитируй читаемый текст («батч читается как 45L310») или опиши видимый дефект с привязкой к месту («зазор между кольцом и стеклом справа»). Без цитаты — ✅ или ➖, никогда ️/❌.
- Отсутствие батча на флаконе само по себе — НЕ ❌ (у части оригиналов кода на дне нет): ❌ по п.1 только при видимых следах вмешательства; отсутствие без следов — максимум ⚠️ с оговоркой. Отсутствие «ожидаемых надписей/логотипов» на коробке — НЕ ❌ без точного знания бренда; п.7 — только конфликт «коробка оригинал ↔ флакон нет».
...
def hint_for(step,ff):
    ...
    if "батч" in s:
        return "Код на дне флакона: гравировка, печать или наклейка; иногда — на боку у основания. Переверните флакон и снимите дно крупным планом, чтобы читался текст и были видны швы стекла."
...
def batch_note(s,n):
    if 1<=n<=len(s["queue"]) and "батч" in s["queue"][n-1].lower() and "короб" not in s["queue"][n-1].lower():
        return ("\nЕсли кода на флаконе нет совсем — напишите «нет», проверим остальные детали." if s.get("box")=="нет" else "\nЕсли кода на флаконе нет совсем — напишите «нет», проверим коробку и остальные детали.")
    return ""
...
def img_b64(m):
    ...
    b64,w,h=downscale_b64(r.content,2048 if is_doc else 1600)
    return b64,(not is_doc),w,h

def st(cid):
    return S.setdefault(cid,{"name":"","photos":[],"shots":"","queue":[],"closed":[],"cannot":[],"last_missing":[],"stage":"name","source":"","tariff":"","ff":"default","pending":-1,"pending_b64":"","retakes":0,"chain_complete":False,"last_closed":-1,"report_text":"","details":{},"rechecks":0,"rc_photo":"","warned_low":False,"box":""})
...
def reset(cid):
    S[cid]={"name":"","photos":[],"shots":"","queue":[],"closed":[],"cannot":[],"last_missing":[],"stage":"name","source":"","tariff":"","ff":"default","pending":-1,"pending_b64":"","retakes":0,"chain_complete":False,"last_closed":-1,"report_text":"","details":{},"rechecks":0,"rc_photo":"","warned_low":False,"box":""}
...
def step_msg(s,ni):
    ...
    if "батч" in s["queue"][ni].lower() and "короб" not in s["queue"][ni].lower():
        h+=(" Если кода на флаконе нет совсем — напишите «нет», проверим остальные детали." if s.get("box")=="нет" else BATCH_NO_NOTE)
    return f"➡️ Шаг {ni+1}/{len(s['queue'])}: {s['queue'][ni]}" + (f"\n{h}" if h else "") + "\nЕсли этот кадр не получается снять — напишите «нет», пропустим шаг и пойдём дальше."
...
def start(m):
    parts=m.text.split()
    S[m.chat.id]={"name":"","photos":[],"shots":"","queue":[],"closed":[],"cannot":[],"last_missing":[],"stage":"name","source":parts[1] if len(parts)>1 else "","tariff":"","ff":"default","pending":-1,"pending_b64":"","retakes":0,"chain_complete":False,"last_closed":-1,"report_text":"","details":{},"rechecks":0,"rc_photo":"","warned_low":False,"box":""}
...
def add_image(m):
    ...
    if s["stage"]=="feedback":
        ...
        return
    try:
        b64,comp,img_w,img_h=img_b64(m)
    except Exception:
        logging.exception("img_b64")
        bot.send_message(cid,"Не смог прочитать файл. Пришлите фото в JPG/PNG документом.")
        return
    if max(img_w,img_h)<2048 and not s.get("warned_low"):
        s["warned_low"]=True
        bot.send_message(cid,f"⚠️ Фото выглядит сжатым (разрешение {img_w}×{img_h}). Батч и полиграфия могут не прочитаться. Что не сможет провериться — в отчёте будет честно помечено «не проверяется».")
    if s["stage"]=="done":
        if comp:
            bot.send_message(cid,"📥 Для перепроверки принимаю фото ТОЛЬКО документом: Telegram сжимает обычные фото, и перепроверка будет неточной. Отправьте документом:\n\n"+DOC_HELP)
            return
...
def text(m):
    ...
    if s["stage"]=="recheck":
        ...
        if s["rechecks"]<recheck_limit(s):
            msg+="\nЕсли хотите перепроверить ещё пункт — пришлите фото этого места документом."
        else:
